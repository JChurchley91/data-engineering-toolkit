...
| `data_pipelines/`       | A Python directory that contains code for your Dagster repository                 |
| `data_pipelines_tests/` | A Python directory that contains tests for `data_pipelines`                      |
...
pyenv virtualenv $PYTHON_VERSION data_pipelines
pyenv activate data_pipelines
```

**Conda**
```bash
export PYTHON_VERSION=X.Y.Z
conda create --name data_pipelines python=PYTHON_VERSION
...
Tests can be found in `data_pipelines_tests` and are run with the following command:

```bash
pytest data_pipelines_tests
```

As you create Dagster ops and graphs, add tests in `data_pipelines_tests/` to check that your
